...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Zoom effect triggered by clicking anywhere on the body
  let zoomedIn = false;
  ...
  // Make sure the profile name changes and shakes
  const nameElement = document.querySelector('.name');
  const bioElement = document.querySelector('.bio');
  const gameLink = document.querySelector('.game-link');
  
  // Change the name and bio text randomly
  setInterval(() => {
    const names = ["hccsri", "haha yes", "new name pls", "who am i"];
    const bios = ["Welcome to my funhouse!", "I love tech and being silly", "I make pranks :)", "Haha April Fools!"];
    
    nameElement.textContent = names[Math.floor(Math.random() * names.length)];
    bioElement.textContent = bios[Math.floor(Math.random() * bios.length)];
  }, 3000); // Change text every 3 seconds

  // Apply shaking animation to game link every time it gets clicked
  gameLink.addEventListener('click', () => {
    gameLink.style.animation = "shake 0.5s ease-in-out";
    setTimeout(() => {
      gameLink.style.animation = ""; // Reset the animation
    }, 500);
  });

  // Make the video overlay appear randomly for fun
  setTimeout(() => {
    const videoContainer = document.getElementById('video-container');
    videoContainer.style.display = 'block';
    const video = document.getElementById('kj-flex-video');
    video.play();
  }, 5000); // Video appears after 5 seconds
